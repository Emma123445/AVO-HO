...
const analyzePagne = async (req, res) => {
  try {
    ...
    const prompt = `
      Tu es un expert en culture africaine et pagnes traditionnels.
      L'image envoyée représente un motif de pagne africain. Donne-moi :
      - Le nom africain du pagne s'il existe
      - Le ou les pays où il est utilisé
      - Le pays d'origine
      - L'histoire de ce pagne
      - Son évolution jusqu'à aujourd'hui au Bénin.
    `;
    ...
    const completion = await xaiClient.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "Tu es un expert culturel africain.",
        },
        {
          role: "user",
          content: [
